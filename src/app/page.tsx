...
import { Tv, Film, Clapperboard, Monitor, Smartphone, Tablet, Laptop, LayoutGrid, Radio, Globe } from "lucide-react";
...
export default function LandingPage() {
  ...
  const [copyrightAcknowledged, setCopyrightAcknowledged] = useState(true);
        ...
        <div className="relative min-h-[100dvh] flex flex-col">
          ...
          <nav className="relative z-10 flex items-center justify-between px-6 sm:px-10 py-5">
            ...
            <div className="flex items-center gap-3">
              ...
              {authChecked && (
                signedIn ? (
                  <button onClick={goHome} className="text-sm text-white/80 hover:text-white px-4 py-2 rounded-xl bg-[#ec1c24] hover:bg-[#d41a20] transition-all font-bold shadow-[0_2px_10px_rgba(236,28,36,0.3)]">
                    Watch Now
                  </button>
                ) : (
                  <button onClick={() => setLoginOpen(true)} className="text-sm text-white/80 hover:text-white px-4 py-2 rounded-xl border border-white/10 hover:border-white/20 transition-all font-semibold">
                    Sign In
                  </button>
                )
              )}
